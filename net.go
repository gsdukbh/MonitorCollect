...
// NetProtoFieldsDb 是用于存储网络协议统计数据的 GORM 模型
type NetProtoFieldsDb struct {
	ID        uint   `gorm:"primaryKey;autoIncrement"`         // 数据库主键
	Host      string `gorm:"type:varchar(100);not null;index"` // 主机名
	Timestamp int64  `gorm:"not null;index"`                   // 数据采集时间戳

	IcmpInAddrMaskReps     int64 `gorm:"column:icmp_inaddrmaskreps"`     // 接收的地址掩码应答数
	IcmpInAddrMasks        int64 `gorm:"column:icmp_inaddrmasks"`        // 接收的地址掩码请求数
	IcmpInCsumErrors       int64 `gorm:"column:icmp_incsumerrors"`       // 接收的校验和错误数
	IcmpInDestUnreachs     int64 `gorm:"column:icmp_indestunreachs"`     // 接收的目标不可达消息数
	IcmpInEchoReps         int64 `gorm:"column:icmp_inechoreps"`         // 接收的回显应答（ping 应答）数
	IcmpInEchos            int64 `gorm:"column:icmp_inechos"`            // 接收的回显请求（ping 请求）数
	IcmpInErrors           int64 `gorm:"column:icmp_inerrors"`           // 接收的 ICMP 错误总数
	IcmpInMsgs             int64 `gorm:"column:icmp_inmsgs"`             // 接收的 ICMP 消息总数
	IcmpInParmProbs        int64 `gorm:"column:icmp_inparmprobs"`        // 接收的参数问题消息数
	IcmpInRedirects        int64 `gorm:"column:icmp_inredirects"`        // 接收的重定向消息数
	IcmpInSrcQuenchs       int64 `gorm:"column:icmp_insrcquenchs"`       // 接收的源抑制消息数
	IcmpInTimeExcds        int64 `gorm:"column:icmp_intimeexcds"`        // 接收的超时消息数
	IcmpInTimestampReps    int64 `gorm:"column:icmp_intimestampreps"`    // 接收的时间戳应答数
	IcmpInTimestamps       int64 `gorm:"column:icmp_intimestamps"`       // 接收的时间戳请求数
	IcmpOutAddrMaskReps    int64 `gorm:"column:icmp_outaddrmaskreps"`    // 发送的地址掩码应答数
	IcmpOutAddrMasks       int64 `gorm:"column:icmp_outaddrmasks"`       // 发送的地址掩码请求数
	IcmpOutDestUnreachs    int64 `gorm:"column:icmp_outdestunreachs"`    // 发送的目标不可达消息数
	IcmpOutEchoReps        int64 `gorm:"column:icmp_outechoreps"`        // 发送的回显应答（ping 应答）数
	IcmpOutEchos           int64 `gorm:"column:icmp_outechos"`           // 发送的回显请求（ping 请求）数
	IcmpOutErrors          int64 `gorm:"column:icmp_outerrors"`          // 发送的 ICMP 错误总数
	IcmpOutMsgs            int64 `gorm:"column:icmp_outmsgs"`            // 发送的 ICMP 消息总数
	IcmpOutParmProbs       int64 `gorm:"column:icmp_outparmprobs"`       // 发送的参数问题消息数
	IcmpOutRateLimitGlobal int64 `gorm:"column:icmp_outratelimitglobal"` // 全局速率限制丢弃的消息数
	IcmpOutRateLimitHost   int64 `gorm:"column:icmp_outratelimithost"`   // 主机速率限制丢弃的消息数
	IcmpOutRedirects       int64 `gorm:"column:icmp_outredirects"`       // 发送的重定向消息数
	IcmpOutSrcQuenchs      int64 `gorm:"column:icmp_outsrcquenchs"`      // 发送的源抑制消息数
	IcmpOutTimeExcds       int64 `gorm:"column:icmp_outtimeexcds"`       // 发送的超时消息数
	IcmpOutTimestampReps   int64 `gorm:"column:icmp_outtimestampreps"`   // 发送的时间戳应答数
	IcmpOutTimestamps      int64 `gorm:"column:icmp_outtimestamps"`      // 发送的时间戳请求数
	IcmpMsgInType0         int64 `gorm:"column:icmpmsg_intype0"`         // 接收的 Type 0 消息数（Echo Reply - 回显应答）
	IcmpMsgInType3         int64 `gorm:"column:icmpmsg_intype3"`         // 接收的 Type 3 消息数（Destination Unreachable - 目标不可达）
	IcmpMsgInType8         int64 `gorm:"column:icmpmsg_intype8"`         // 接收的 Type 8 消息数（Echo Request - 回显请求）
	IcmpMsgInType11        int64 `gorm:"column:icmpmsg_intype11"`        // 接收的 Type 11 消息数（Time Exceeded - 超时）
	IcmpMsgOutType0        int64 `gorm:"column:icmpmsg_outtype0"`        // 发送的 Type 0 消息数（Echo Reply - 回显应答）
	IcmpMsgOutType3        int64 `gorm:"column:icmpmsg_outtype3"`        // 发送的 Type 3 消息数（Destination Unreachable - 目标不可达）
	IcmpMsgOutType8        int64 `gorm:"column:icmpmsg_outtype8"`        // 发送的 Type 8 消息数（Echo Request - 回显请求）
	IpDefaultTTL           int64 `gorm:"column:ip_defaultttl"`           // 默认 TTL（Time To Live）值
	IpForwarding           int64 `gorm:"column:ip_forwarding"`           // IP 转发开关（1=开启，0=关闭）
	IpForwDatagrams        int64 `gorm:"column:ip_forwdatagrams"`        // 转发的数据报数量
	IpFragCreates          int64 `gorm:"column:ip_fragcreates"`          // 创建的 IP 分片数量
	IpFragFails            int64 `gorm:"column:ip_fragfails"`            // 分片失败的数据报数量
	IpFragOKs              int64 `gorm:"column:ip_fragoks"`              // 成功分片的数据报数量
	IpInAddrErrors         int64 `gorm:"column:ip_inaddrerrors"`         // 接收的地址错误数据报数量
	IpInDelivers           int64 `gorm:"column:ip_indelivers"`           // 成功交付到上层协议的数据报数量
	IpInDiscards           int64 `gorm:"column:ip_indiscards"`           // 接收时丢弃的数据报数量
	IpInHdrErrors          int64 `gorm:"column:ip_inhdrerrors"`          // 接收的头部错误数据报数量
	IpInReceives           int64 `gorm:"column:ip_inreceives"`           // 接收的 IP 数据报总数
	IpInUnknownProtos      int64 `gorm:"column:ip_inunknownprotos"`      // 接收的未知协议数据报数量
	IpOutDiscards          int64 `gorm:"column:ip_outdiscards"`          // 发送时丢弃的数据报数量
	IpOutNoRoutes          int64 `gorm:"column:ip_outnoroutes"`          // 因无路由而丢弃的数据报数量
	IpOutRequests          int64 `gorm:"column:ip_outrequests"`          // 本地上层协议请求发送的数据报数量
	IpOutTransmits         int64 `gorm:"column:ip_outtransmits"`         // 成功发送的 IP 数据报总数
	IpReasmFails           int64 `gorm:"column:ip_reasmfails"`           // IP 重组失败的次数
	IpReasmOKs             int64 `gorm:"column:ip_reasmoks"`             // IP 重组成功的次数
	IpReasmReqds           int64 `gorm:"column:ip_reasmreqds"`           // 需要重组的 IP 分片数量
	IpReasmTimeout         int64 `gorm:"column:ip_reasmtimeout"`         // IP 重组超时的次数
	TcpActiveOpens         int64 `gorm:"column:tcp_activeopens"`         // 主动打开的 TCP 连接数（客户端发起）
	TcpAttemptFails        int64 `gorm:"column:tcp_attemptfails"`        // 连接尝试失败的次数
	TcpCurrEstab           int64 `gorm:"column:tcp_currestab"`           // 当前已建立的 TCP 连接数
	TcpEstabResets         int64 `gorm:"column:tcp_estabresets"`         // 已建立连接被重置的次数
	TcpInCsumErrors        int64 `gorm:"column:tcp_incsumerrors"`        // 接收的校验和错误数
	TcpInErrs              int64 `gorm:"column:tcp_inerrs"`              // 接收的 TCP 错误总数
	TcpInSegs              int64 `gorm:"column:tcp_insegs"`              // 接收的 TCP 段总数
	TcpMaxConn             int64 `gorm:"column:tcp_maxconn"`             // TCP 最大连接数（-1 表示动态）
	TcpOutRsts             int64 `gorm:"column:tcp_outrsts"`             // 发送的 RST 段数量
	TcpOutSegs             int64 `gorm:"column:tcp_outsegs"`             // 发送的 TCP 段总数
	TcpPassiveOpens        int64 `gorm:"column:tcp_passiveopens"`        // 被动打开的 TCP 连接数（服务端接受）
	TcpRetransSegs         int64 `gorm:"column:tcp_retranssegs"`         // 重传的 TCP 段数量
	TcpRtoAlgorithm        int64 `gorm:"column:tcp_rtoalgorithm"`        // RTO（重传超时）算法编号
	TcpRtoMax              int64 `gorm:"column:tcp_rtomax"`              // 最大 RTO 值（毫秒）
	TcpRtoMin              int64 `gorm:"column:tcp_rtomin"`              // 最小 RTO 值（毫秒）
	UdpIgnoredMulti        int64 `gorm:"column:udp_ignoredmulti"`        // 忽略的多播数据报数量
	UdpInCsumErrors        int64 `gorm:"column:udp_incsumerrors"`        // 接收的校验和错误数
	UdpInDatagrams         int64 `gorm:"column:udp_indatagrams"`         // 接收的 UDP 数据报总数
	UdpInErrors            int64 `gorm:"column:udp_inerrors"`            // 接收的 UDP 错误总数
	UdpMemErrors           int64 `gorm:"column:udp_memerrors"`           // UDP 内存分配错误数
	UdpNoPorts             int64 `gorm:"column:udp_noports"`             // 发送到无监听端口的数据报数量
	UdpOutDatagrams        int64 `gorm:"column:udp_outdatagrams"`        // 发送的 UDP 数据报总数
	UdpRcvbufErrors        int64 `gorm:"column:udp_rcvbuferrors"`        // 接收缓冲区溢出错误数
	UdpSndbufErrors        int64 `gorm:"column:udp_sndbuferrors"`        // 发送缓冲区溢出错误数
	UdpliteIgnoredMulti    int64 `gorm:"column:udplite_ignoredmulti"`    // 忽略的 UDPLite 多播数据报数量
	UdpliteInCsumErrors    int64 `gorm:"column:udplite_incsumerrors"`    // 接收的 UDPLite 校验和错误数
	UdpliteInDatagrams     int64 `gorm:"column:udplite_indatagrams"`     // 接收的 UDPLite 数据报总数
	UdpliteInErrors        int64 `gorm:"column:udplite_inerrors"`        // 接收的 UDPLite 错误总数
	UdpliteMemErrors       int64 `gorm:"column:udplite_memerrors"`       // UDPLite 内存分配错误数
	UdpliteNoPorts         int64 `gorm:"column:udplite_noports"`         // 发送到无监听端口的 UDPLite 数据报数量
	UdpliteOutDatagrams    int64 `gorm:"column:udplite_outdatagrams"`    // 发送的 UDPLite 数据报总数
	UdpliteRcvbufErrors    int64 `gorm:"column:udplite_rcvbuferrors"`    // UDPLite 接收缓冲区溢出错误数
	UdpliteSndbufErrors    int64 `gorm:"column:udplite_sndbuferrors"`    // UDPLite 发送缓冲区溢出错误数

	CreatedAt int64 `gorm:"autoCreateTime"` // 记录创建时间
	UpdatedAt int64 `gorm:"autoUpdateTime"` // 记录更新时间
}

// TableName 指定 NetProtoFieldsDb 的表名
func (NetProtoFieldsDb) TableName() string {
	return "net_protocol_metrics"
}

// FromNetProtoFields 从 NetProtoFields 和标签填充 NetProtoFieldsDb
func (db *NetProtoFieldsDb) FromNetProtoFields(host string, timestamp int64, fields NetProtoFields) {
	db.Host = host
	db.Timestamp = timestamp
	// 使用 reflect 简化字段赋值
	...
	for i := 0; i < fieldsValue.NumField(); i++ {
		...
		dbField := dbValue.FieldByName(fieldName)
		if dbField.IsValid() && dbField.CanSet() {
			dbField.Set(fieldsValue.Field(i))
		}
	}
}
...
func SaveNetToDB(metric *TelegrafJson) {
	...
	if isInterfaceMetric && iface != "all" {
		...
		// 处理协议数据
		var protoFields NetProtoFields
		if err := protoFields.FromFieldsMap(metric.Fields); err != nil {
			log.Printf("解析网络协议字段出错: %v", err)
			return
		}
		var protoDb NetProtoFieldsDb
		protoDb.FromNetProtoFields(
			metric.Tags["host"],
			metric.Timestamp,
			protoFields,
		)
